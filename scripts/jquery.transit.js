...
(function($) {
  ...
  $.cssHooks.transform = {
    ...
    get: function(elem) {
      // https://github.com/zhenglong/jquery.transit/commit/5641cad804af8e8828542f8d9b6505c5ff22bb29
  ...
  function getTransitionMerged(properties, duration, easing, delay, self) {
    // Get the CSS properties needed.
	  ...
	  var allTransitions = self.get(0).style[support.transition].split(/(?![^(]*\)),/); // splits by comma unless the comma is in parenthesis - to avoid splitting cubic-bezier(a,b,c,d)
	  ...
	  for(var i=0;i<allTransitions.length;i++) {
		  if(allTransitions[i].indexOf('none') != -1) {
  ...
  $.fn.transition = $.fn.transit = function(properties, duration, easing, callback) {
    ...
    // Build the `transition` property. - ANNAM - im moving this into the run() function because it checkes the existing transitions on runtime
    ...
    // ANNAM - not using this anymore, delete
    // var oldTransitions = {};

    var run = function(nextCall) {
      var bound = false;
      
      // ANNAM - new getTransition() position (to get existing transitions on runtime) & changed function call
      var transitionValue = getTransitionMerged(properties, duration, easing, delay, self);
      ...
      var cb = function() {
        ...
        if (i > 0) {
          self.each(function() {
            // ANNAM - unmerge transition, keep the other transition properties instead of emptying
  ...
  // ANNAM - $.transit.getTransitionValue = getTransition;
